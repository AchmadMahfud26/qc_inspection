...
try {
    ...
    $header_sql = "SELECT inspection_no FROM inspection_headers WHERE id = $id AND inspection_type = 'After Painting'";
    ...
    $delete_sql = "DELETE FROM inspection_headers WHERE id = $id AND inspection_type = 'After Painting'";
    if (!$db->query($delete_sql)) {
        throw new Exception('Error deleting inspection: ' . $db->error);
    }
    
    // Log activity
    logActivity($db, $_SESSION['user']['id'], 'Delete', 'After Painting Inspection', $inspection_no);
