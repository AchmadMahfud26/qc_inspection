...
if (!in_array($_SESSION['user']['role'], ['admin', 'qc_inspector'])) {
    header('Location: ' . BASE_URL . '/inspection/final_check/');
    exit;
}

$page_title = 'Tambah Final Check Inspection';
    ...
    if (empty($errors)) {
        try {
            ...
            $seq_sql = "SELECT COALESCE(MAX(CAST(SUBSTRING(inspection_no, -4) AS UNSIGNED)), 0) + 1 as next_seq 
                       ...
                       WHERE inspection_type = 'Final Check' 
            ...
            $inspection_no = 'FC-' . $inspection_date_formatted . '-' . str_pad($next_seq, 4, '0', STR_PAD_LEFT);
            ...
            $header_sql = "INSERT INTO inspection_headers 
                          ...
                          VALUES 
                          ('$inspection_no', 'Final Check', '$inspection_date', '$inspection_time',
            ...
            // Log activity
            logActivity($db, $_SESSION['user']['id'], 'Create', 'Final Check Inspection', $inspection_no);
...
// Get inspection items for Final Check
$items_sql = "SELECT id, item_name, standard, inspection_method, sequence 
             FROM inspection_items 
             WHERE process_type = 'Final Check' AND status = 'active'
             ORDER BY sequence";
...
<div class="container-fluid pt-4">
    <div class="row mb-4">
        <div class="col-md-12">
            <h2 class="page-title">Tambah Final Check Inspection</h2>
            <p class="text-muted">Form input hasil inspeksi produk sebelum pengiriman ke customer</p>
        </div>
    </div>
    ...
    <form method="POST" enctype="multipart/form-data" id="inspectionForm">
        ...
        <div class="card mb-4">
            <div class="card-header bg-success text-white">
                <h5 class="mb-0"><i class="fas fa-checklist"></i> Checklist Inspeksi Final Check</h5>
            </div>
            <div class="card-body">
