...
try {
    ...
    $header_sql = "SELECT inspection_no FROM inspection_headers WHERE id = $id AND inspection_type = 'After Welding'";
    ...
    $delete_sql = "DELETE FROM inspection_headers WHERE id = $id AND inspection_type = 'After Welding'";
    if (!$db->query($delete_sql)) {
        throw new Exception('Error deleting inspection: ' . $db->error);
    }
    
    // Log activity
    logActivity($db, $_SESSION['user']['id'], 'Delete', 'After Welding Inspection', $inspection_no);
